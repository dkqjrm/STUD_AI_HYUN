...
import numpy as np
import random
from transformers import AutoTokenizer, AutoAdapterModel, AdamW, BertConfig, BertModelWithHeads
...
class Classifier(LightningModule):
    def __init__(self, config):  # drop_prob를 활용하여 dropout 추가해볼 것.
        ...
        self.model = AutoAdapterModel.from_pretrained("klue/bert-base")

        adapterconfig = AdapterConfig(mh_adapter=True, output_adapter=True, reduction_factor=self.config['reduction'],
                                      non_linearity="relu")
        self.model.add_adapter("dropout", config=adapterconfig)
        self.model.add_classification_head(
            "dropout",
            num_labels=2,
            id2label={0: "no_dropout", 1: "yes_dropout"}
        )
    ...
    def forward(self, x):
        ...
        out = self.model(attention_mask = x[1],
                                   inputs_embeds=inputs_embeds).logits  # [batch_size, 768]
        return out
